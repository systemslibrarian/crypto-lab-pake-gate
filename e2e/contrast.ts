 ...
 *    an unknown backdrop, or a `color-mix()`. This page has ten of them, and
 *    they are not decorative: `.cl-hero-why` (the "WHY IT MATTERS" aside, the
 *    first prose a reader meets) is `color-mix(in oklab, var(--accent) 6%,
 *    transparent)`; every `.wcard__caption` — the plain-language "what just
 *    happened" line that leads each wire message — is
 *    `color-mix(in srgb, var(--accent) 8%, transparent)`; `.matrix__row--ref`
 *    tints the reference row of the taxonomy table the same way; and
 *    `.plot__pick.is-active` tints the selected Dragonblood candidate. axe files
 *    all of them under `incomplete`, so a violations-only gate never measured
 *    the contrast of this lab's own explanatory copy.
 ...
 *    at `.85`, and the two inactive-component cases `.btn:disabled` at `.45` and
 *    `.tab:disabled` at `.5` — the latter covering three of the four protocol
 *    tabs for as long as the lab is in its shipped guided view.
 *
 ...
 * not behind it. That is live on this page: `.srow__tip`, the glossary tooltip
 * on every notation term, is `position: absolute` at `top: calc(100% + 0.3rem)`
 * — it paints entirely BELOW its `.srow__label` host, on whatever happens to be
 * under it, and its own `--panel` fill is what actually backs its text. So an
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. This lab is full
 *    of them: every `.hexbox__value` is `overflow-x: auto; white-space: nowrap`
 *    around a full hex value, `.attacker__raw` caps the raw transcript at 360px
 *    with `overflow: auto`, and `.taxonomy__scroll` / `.dblood__scroll` clip a
 *    900px table and a `max-content` plot. Content scrolled past those is not
 ...
 *    draws its one figure — the Dragonblood iteration plot — in HTML and flexbox
 *    rather than SVG, so the sibling walk finds nothing to do here; the only SVG
 *    on the page is the shared header's two `aria-hidden` glyphs. It is kept
 ...
 * not one — nor is it one to axe's `color-contrast` rule. This lab's own
 * stylesheet declares no pseudo-element content at all (its only `::before` /
 * `::after` mention is the reduced-motion reset), and neither does the shared
 * header, so the gap is currently empty here. It is covered anyway, by
 * `nontext.ts`'s second check, which reads `content` off every pseudo on the
 * page and measures its ink — because "currently empty" is a property of
 * today's stylesheet, not of the page.
 */
 ...
 * uses. This lab has two genuinely short-lived styles: `.wcard--new`, the
 * 0.6s lift on the newest wire card, and `.scratch--stayed`, the 1.4s
 * lock-pulse on the peer that just published. Under the reduced motion this
 * gate asserts, both animations are clamped to 0.01ms and land on their end
 * state immediately, so neither paints anything transient — and `scan` waits
 * for the 1.4s timer that REMOVES `.scratch--stayed` before it measures, so the
 * rendering it walks is the one a reader is left looking at rather than a frame
 * caught mid-flight.
 *
 * The parameter is kept because the narrowed form is what makes a short-lived
 * state measurable at all: a class that lives 900ms — which is exactly how long
 * `dom.ts` leaves a copy button reading "copied" — is shorter than a full axe
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A driven pass here walks four protocol tabs, a five-row taxonomy matrix
     * nine columns wide, and — the expensive part — the on-path observer's raw
     * transcript, which renders every field of every wire message as its own
     * `.rawmsg__field` block, all of them re-walking the same ancestors up to
     ...
     * pass, so the cached values cannot go stale — `scan` waits out this lab's
     * one self-scheduled re-render (the 1400ms `.scratch--stayed` pulse timer)
     * before it starts.
     */
     ...
     * that is every `.hexbox__value` (`overflow-x: auto; white-space: nowrap`
     * around a full hex value), `.hexbox` itself (`overflow: hidden`, which is
     * what rounds the box's corners over the copy button), `.attacker__raw`
     * (`max-height: 360px; overflow: auto` over the whole raw transcript),
     * `.taxonomy__scroll` (a 900px matrix), `.dblood__scroll` (a `max-content`
     * plot), `.rawmsg__hex`, `.breach__dump-row code` and the `.tabs` strip.
     */
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. `checkVisibility()` catches it. This lab ships no
      // `<details>` at all, but it hides text the same way through CSS: every
      // `.srow__tip` glossary tooltip is `display: none` until its label is
      // hovered or focused, and `.aux:not(:empty)` only lays out once a panel
      // exists. The guard is what keeps those out of the walk without the walk
      // having to know which of them is which.
      ...
      //
      // This lab does NOT currently use the idiom — neither `styles.css` nor
      // `index.html` defines a `.sr-only` class, and that was checked rather
      // than assumed, because the consequence of the guard being absent when
      // the idiom arrives is a fabricated failure rather than a missed one: a
      // zero-area clipped element keeps a 1x1 rect and passes
      // `checkVisibility()` while the compositor draws nothing, so it would be
      // measured as ink that was never laid down. Its three live `aria-live`
      // regions (`.status`, `.dblood__explain`, and every `role="alert"`
      // verdict) are all fully VISIBLE announcements rather than screen-reader
      // -only ones, which is why nothing here is clipped away today. The guard
     ...
     * `.verdict__icon` (✓ / ⚠ / …), `.scratch__lock` (🔒), `.wcard__caption-icon`
     * (✉), and the shared header's two `<svg>` marks. In every case the words
     * that carry the meaning sit immediately beside the glyph, in the same ink
     * on the same surface, so measuring the sibling measures the glyph too —
     * `.verdict__text` is the sentence beside the ✓, `.scratch__banner`'s
     * "private — never leaves this box" is the sentence beside the 🔒.
     *
     * Nothing on this page hides a VALUE. No derived key, no fingerprint, no
     * wire field, no verdict text and no audit result is inside an
     ...
     *
     * This lab has no SVG of its own — its one figure, the Dragonblood
     * iteration plot, is flexbox `<div>`s with `height` percentages, and the
     * only SVG on the page is the shared header's two `aria-hidden` marks. The
     * guard stays because a walk that measured a non-rendering SVG text node
     * would report a 1:1 failure describing ink that was never laid down, and
     * the day this lab draws its first real chart is the day that starts
     * mattering.
     */
     ...
     *
     * Here it is currently load-bearing in the good direction: `styles.css`
     * sets `html { background-color: var(--bg) }` explicitly, with a comment
     * saying it exists so the body background reaches the canvas at all. That
     * declaration is one cleanup away from being deleted as redundant, and this
     * page runs many screens deep with four tabs of panels — so the day it goes,
