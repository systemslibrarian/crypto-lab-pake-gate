...
export class JPakeRunner extends StepMachine implements Runner {
  ...
  private build(): void {
    ...
    this.stages.push(
      { label: "A → Round 1", run: () => { a1 = this.maybeTamper(this.a.round1()); return (a1c = this.push(a1, ["g1", "g2", "V1", "r1", "V2", "r2"])); } },
      { label: "B → Round 1", run: () => { b1 = this.maybeTamper(this.b.round1()); return (b1c = this.push(b1, ["g3", "g4", "V3", "r3", "V4", "r4"])); } },
      ...
      { label: "A → Round 2", run: () => { a2 = this.maybeTamper(this.a.round2()); return (a2c = this.push(a2, ["elem", "V", "r"])); } },
      { label: "B → Round 2", run: () => { b2 = this.maybeTamper(this.b.round2()); return (b2c = this.push(b2, ["elem", "V", "r"])); } },
  ...
  private peerView(p: JPakeParty, title: string, self: string, peer: string): PeerView {
    ...
    const scratch: ScratchRow[] = [
      { label: "s = scalar(password)", value: bigHex(t.s), secret: true },
      { label: "K_material", value: t.kMaterial ? bytesHex(t.kMaterial) : "—", secret: true },
      { label: "kc (confirm key)", value: t.kc ? bytesHex(t.kc) : "—", secret: true },
      { label: "gCombined (self)", value: t.gCombinedSelf !== undefined ? bigHex(t.gCombinedSelf) : "—", secret: false },
      { label: "own element", value: t.elemSelf !== undefined ? bigHex(t.elemSelf) : "—", secret: false },
    ];
